                    ...
                    @foreach ($walis as $wali)
                        ...
                        <tr class="hover:bg-blue-50 transition">
                            <td class="px-4 py-3 text-center">
                                <input type="checkbox" value="{{ $wali->id }}" x-model="selectedItems" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <form action="{{ route('wali.destroy', $wali->id) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
...
<script>
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('waliHandler', () => ({
            ...
            selectedItems: [],
            toggleAll() {
                this.selectedItems = this.selectAll ? @json($walis->pluck('id')) : [];
            },
            ...
            openModalOnValidationFailure() {
                ...
                this.modalTitle = isEdit ? 'Edit Data Wali (Kesalahan Input)' : 'Tambah Wali Baru (Kesalahan Input)';
                
                // Cari ID wali yang sedang diedit (untuk URL action)
                let waliId = @json(request()->route('wali')?->id);
                if(isEdit && waliId) {
                    this.modalActionUrl = `/wali/${waliId}`;
                } else {
