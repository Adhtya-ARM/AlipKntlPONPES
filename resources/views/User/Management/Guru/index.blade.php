        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-100">
            ...
            <div class="flex flex-wrap gap-2">
                <button @click="openCreateModal" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-sm flex items-center gap-2">
                    <i class="fas fa-plus"></i> Guru Baru
                </button>
            </div>
        </div>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($gurus as $guru)
                        @php
                            ...
                            $isLocked = ($profile && ($profile->guru_mapels_count > 0 || $profile->kelas_wali_count > 0));
                        ...
                        <tr class="hover:bg-blue-50 transition">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    @else
                                        <button @click="deleteGuru({{ $guru->id }}, '{{ $profile->nama ?? $guru->username }}')" class="text-red-600 hover:bg-red-100 p-1.5 rounded-md transition" title="Hapus">
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden transform transition-all" @click.away="isModalOpen = false">
            ...
            <form @submit.prevent="submitForm" class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
                ...
                <div>
                    ...
                    <input type="text" x-model="formData.jabatan" required class="w-full border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3">
                </div>
        ...
        Alpine.data('guruHandler', () => ({
            ...
            showCannotDeleteInfo() {
                Swal.fire({
                    ...
                    text: 'Guru ini masih memiliki relasi aktif (mengajar mapel atau menjadi wali kelas). Silakan hapus relasi tersebut terlebih dahulu.'
                });
            }
        }));
    });
