...
<div x-data="penilaianApp()">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            ...
            {{-- Tombol Tugas Harian --}}
            <button @click="isTugasHarianModalOpen = true"
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr x-data='{ santriData: @json($santriAlpineData) }'>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $santri->nama ?? '-' }}
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div class="flex items-center space-x-2 justify-center">
                            ...
                            <form action="{{ route('penilaian.destroy', $nilaiMapel->id) }}" method="POST"
    ...
    <div x-show="isModalOpen" x-cloak x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div x-transition:enter="ease-out duration-300"
                ...
                class="bg-white rounded-xl shadow-2xl transform transition-all max-w-md w-full mx-4 my-8" @click.stop>
                ...
                <div class="p-6 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                    ...
                    <p class="text-sm text-gray-500 mt-1" x-show="!isDetailMode">Masukkan atau perbarui nilai (0-100)
                        untuk mata pelajaran yang relevan.</p>
                ...
                <form :action="modalActionUrl" method="POST" class="p-6" x-show="!isDetailMode">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4 pb-4 border-b border-gray-200">
                            <div>
                                ...
                                <p x-ref="nis_form"
                                    class="mt-1 block w-full border-0 p-0 text-sm font-semibold text-gray-900 bg-white"
                                    x-text="modalData ? modalData.nis : ''"></p>
                            </div>
                            <div>
                        ...
                        <div>
                            <label for="nilai_harian" class="block text-sm font-medium text-gray-700">Nilai Harian (DB:
                                nilai)</label>
                            <input type="number" x-ref="nilai_harian_input" id="nilai_harian" name="nilai_harian"
                        ...
                        <div>
                            <label for="nilai_uts" class="block text-sm font-medium text-gray-700">Nilai UTS (DB:
                                uts)</label>
                            <input type="number" x-ref="nilai_uts_input" id="nilai_uts" name="nilai_uts" min="0"
                                max="100" placeholder="Contoh: 78"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                                required>
                        </div>

                        <div>
                            <label for="nilai_uas" class="block text-sm font-medium text-gray-700">Nilai UAS (DB:
                                uas)</label>
                            <input type="number" x-ref="nilai_uas_input" id="nilai_uas" name="nilai_uas" min="0"
                                max="100" placeholder="Contoh: 92"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                                required>
                        </div>

                        <div class="pt-2">
                            <label for="catatan" class="block text-sm font-medium text-gray-700">Catatan Pengajar (DB:
                                catatan) (Opsional)</label>
                            <textarea x-ref="catatan_input" id="catatan" name="catatan" rows="2"
                                placeholder="Tuliskan catatan khusus terkait performa santri..."
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button type="submit"
                            class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-md transition duration-150">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-bold text-gray-800"
                                x-text="modalData && modalData.nilai_harian !== null ? modalData.nilai_harian : '-'">
                            </p>
                        </div>
                        ...
                        <div class="pt-2">
                            ...
                            <p class="text-sm text-gray-700 italic border-l-4 border-indigo-400 pl-3 py-1"
                                x-text="modalData && modalData.catatan && modalData.catatan !== '' ? modalData.catatan : 'Tidak ada catatan.'">
                            </p>
                        </div>
                    </div>
    ...
    <div x-show="isUploadModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white rounded-xl shadow-2xl max-w-sm w-full mx-4 my-8" @click.stop>
                <div class="p-6 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                    <h3 class="text-xl font-bold text-indigo-700">Upload Nilai dari PDF</h3>
                    <p class="text-sm text-gray-500 mt-1">Pastikan format PDF sesuai dengan format parsing di backend.
                    </p>
                </div>

                <form action="{{ route('penilaian.upload.pdf') }}" method="POST" enctype="multipart/form-data"
                    class="p-6">
                    @csrf
                    <div class="space-y-4">
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        <button type="button" @click="isUploadModalOpen = false"
                            class="px-4 py-2 text-sm font-medium text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-100">Batal</button>
                        <button type="submit"
                            class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Upload
                            & Proses</button>
                    </div>
                </form>
            </div>
    ...
    <div x-show="isTugasHarianModalOpen" x-cloak x-transition:enter="ease-out duration-300"
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full mx-4 my-8" @click.stop>

                <div class="p-6 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                    <h3 class="text-xl font-bold text-purple-700">Input Tugas Harian</h3>
                    <p class="text-sm text-gray-500 mt-1">Masukkan nilai tugas harian untuk 5 bab mata pelajaran.</p>
                </div>

                <form action="{{ route('penilaian.store') }}" method="POST" class="p-6">
                    @csrf
                    ...
                    <div class="space-y-4">
                        {{-- Pilih Santri --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Pilih Santri</label>
                            <select name="santri_profile_id"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500"
                                required>
                                <option value="">-- Pilih Santri --</option>
                                @foreach ($santriProfiles as $santri)
                                <option value="{{ $santri->id }}">{{ $santri->nama }} (NIS:
                                    {{ $santri->santri->nis ?? '-' }})</option>
                                @endforeach
                            </select>
                        </div>

                        {{-- Tabel Input Nilai Bab --}}
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 border border-gray-300">
                                <thead class="bg-purple-50">
                                    <tr>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-purple-700 uppercase">
                                            Bab</th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-purple-700 uppercase">
                                            Nilai (0-100)</th>
                                    </tr>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr>
                                        <td class="px-4 py-2 text-sm font-medium text-gray-900">Bab 1</td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="bab1" min="0" max="100" placeholder="0"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="px-4 py-2 text-sm font-medium text-gray-900">Bab 2</td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="bab2" min="0" max="100" placeholder="0"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="px-4 py-2 text-sm font-medium text-gray-900">Bab 3</td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="bab3" min="0" max="100" placeholder="0"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="px-4 py-2 text-sm font-medium text-gray-900">Bab 4</td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="bab4" min="0" max="100" placeholder="0"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="px-4 py-2 text-sm font-medium text-gray-900">Bab 5</td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="bab5" min="0" max="100" placeholder="0"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm">
                                        </td>
                                    </tr>
                                </tbody>
                        ...
                        {{-- Catatan --}}
                        <div>
                            <label for="catatan_tugas" class="block text-sm font-medium text-gray-700">Catatan
                                (Opsional)</label>
                            <textarea id="catatan_tugas" name="catatan" rows="2"
                                placeholder="Catatan untuk tugas harian..."
...
document.addEventListener('alpine:init', () => {
    Alpine.data('penilaianApp', () => ({
        ...
        isTugasHarianModalOpen: false, // Modal Tugas Harian
        modalTitle: '',
        modalActionUrl: '',
        modalMethod: 'POST',
        isDetailMode: false,
        ...
        openCreateModal(santri) {
            ...
            this.$nextTick(() => {
                ...
                if (this.$refs.nama_form) this.$refs.nama_form.innerText = santri.nama ??
                    'N/A';
            });
        },
        ...
        fillAssessmentForm(santri) {
            ...
            if (this.$refs.nilai_harian_input) this.$refs.nilai_harian_input.value = santri
                .nilai_harian ?? '';
